...
```flowchart
flowchart TD
  A["첫 실행 — SPIR-V 컴파일 · 느림"]
  B(["Pipeline Cache 저장 — vkGetPipelineCacheData"])
  C["이후 실행 — 캐시 히트 · 빠름"]
  A --> B --> C
```
...
```flowchart
flowchart TD
  A["CPU Thread 1 — Secondary CB (지형)"]
  B["CPU Thread 2 — Secondary CB (오브젝트)"]
  C["Primary CB — ExecuteCommands × 2"]
  D(["Queue Submit"])
  A --> C
  B --> C
  C --> D
